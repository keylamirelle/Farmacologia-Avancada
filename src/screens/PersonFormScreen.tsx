...
type Nav = NativeStackNavigationProp<RootStackParamList, 'PersonForm'>;
type Route = RouteProp<RootStackParamList, 'PersonForm'>;
...
export function PersonFormScreen() {
  ...
  async function save() {
    if (!name.trim()) return Alert.alert('Informe o nome');
    setSaving(true);
    const payload = {
      name: name.trim(),
      role: role.trim() || null,
  ...
  async function remove() {
    ...
    Alert.alert(
      'Excluir pessoa',
      'A pessoa será removida do cadastro. Retiradas já registradas em nome dela permanecem no histórico. Continuar?',
      [
          ...
          onPress: async () => {
            ...
            if (error) {
              Alert.alert(
                ...
                'Provavelmente esta pessoa tem retiradas registradas. Você pode marcar como inativa em vez de excluir.'
              );
            } else {
              nav.goBack();
            }
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={{ padding: 16 }}>
      ...
      <TextInput style={styles.input} value={name} onChangeText={setName} placeholder="Ex: Maria Silva" />

      <Text style={styles.label}>Função / cargo (opcional)</Text>
      <TextInput
        style={styles.input}
        value={role}
      ...
      <View style={styles.switchRow}>
        <View style={{ flex: 1 }}>
          ...
          <Text style={styles.switchHelp}>
            Inativas não aparecem como opção em novas retiradas
          </Text>
        </View>
